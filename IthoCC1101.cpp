...
void IthoCC1101::parseMessageCommand()
{
  ...
  message2Decode(&inMessage2, &inIthoPacket);

  uint8_t deviceType = inIthoPacket.dataDecoded[0];
  //device id
  memcpy(inIthoPacket.deviceId2, &inMessage2.data[2], sizeof inIthoPacket.deviceId2);

  //counter1
  //inIthoPacket.counter = inIthoPacket.dataDecoded[4];
  inIthoPacket.counter = calculateMessageCounter(inMessage2.data[10], inMessage2.data[11], (inMessage2.data[10] & 0b11110000));

  //match received commandBytes from dataDecoded [offset is +5] with known command bytes
  //the last 3 bytes seem to be unique and universal
  uint8_t offset = 0;
  if(deviceType == 28 || deviceType == 24) offset = 2;
  for (int i = 0; i < 6; i++)
  {
    if (i == 2) continue; //skip byte3, rft-rv device seem to sometimes have a different number there for Timer command
    if (inIthoPacket.dataDecoded[i + 5 + offset] != ithoMessage2DecPowerCommandBytes[i])   isPowerCommand    = false;
    if (inIthoPacket.dataDecoded[i + 5 + offset] != ithoMessage2DecHighCommandBytes[i])    isHighCommand     = false;
    if (inIthoPacket.dataDecoded[i + 5 + offset] != ithoMessage2RVHighCommandBytes[i])     isRVHighCommand   = false;
    if (inIthoPacket.dataDecoded[i + 5 + offset] != ithoMessage2DecMediumCommandBytes[i])  isMediumCommand   = false;
    if (inIthoPacket.dataDecoded[i + 5 + offset] != ithoMessage2RVMediumCommandBytes[i])   isRVMediumCommand = false;    
    if (inIthoPacket.dataDecoded[i + 5 + offset] != ithoMessage2DecLowCommandBytes[i])     isLowCommand      = false;
    if (inIthoPacket.dataDecoded[i + 5 + offset] != ithoMessage2RVLowCommandBytes[i])      isRVLowCommand    = false;
    if (inIthoPacket.dataDecoded[i + 5 + offset] != ithoMessage2RVAutoCommandBytes[i])     isRVAutoCommand   = false;
    if (inIthoPacket.dataDecoded[i + 5 + offset] != ithoMessage2DecStandByCommandBytes[i]) isStandByCommand  = false;
    if (inIthoPacket.dataDecoded[i + 5 + offset] != ithoMessage2DecTimer1CommandBytes[i])  isTimer1Command   = false;
    if (inIthoPacket.dataDecoded[i + 5 + offset] != ithoMessage2DecTimer2CommandBytes[i])  isTimer2Command   = false;
    if (inIthoPacket.dataDecoded[i + 5 + offset] != ithoMessage2DecTimer3CommandBytes[i])  isTimer3Command   = false;
    if (inIthoPacket.dataDecoded[i + 5 + offset] != ithoMessage2DecJoinCommandBytes[i])    isJoinCommand     = false;
    if (inIthoPacket.dataDecoded[i + 5 + offset] != ithoMessage2DecJoin2CommandBytes[i])   isJoin2Command    = false;
    if (inIthoPacket.dataDecoded[i + 5 + offset] != ithoMessage2RVJoinCommandBytes[i])     isRVJoinCommand   = false;
    if (inIthoPacket.dataDecoded[i + 5 + offset] != ithoMessage2DecLeaveCommandBytes[i])   isLeaveCommand    = false;
  }
  //determine command
...
void IthoCC1101::message2Decode(CC1101Packet *packet, IthoPacket *itho) {
...
String IthoCC1101::getLastMessage2CMDstr() {
  ...
  //str = "len=" + String(inMessage2.length) + " / cmd:";
  for (uint8_t i = 0; i < inMessage2.length; i++) {
...
String IthoCC1101::LastMessageDecode() {
